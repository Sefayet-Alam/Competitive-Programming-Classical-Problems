...
struct segment_tree{
    ...
    void set(ll i,item val,ll x,ll lx,ll rx){
        if(rx-lx==1){
            //leaf
            tree[x]=single(val);
            return;
...
int main()
{
    ...
    //   for(ll i=0;i<n;i++){
    //         cout<<vec[i].a<<" "<<vec[i].b<<nn;
    //          cout<<vec[i].c<<" "<<vec[i].d<<nn;
    //          cout<<nn;
    //   }
      ...
      while(m--){
        cin>>l>>r;
        l--;
        item ret=sg.calc(l,r);
        
        cout<<ret.a<<" "<<ret.b<<nn<<ret.c<<" "<<ret.d<<nn;
        cout<<nn;
      }
    }
