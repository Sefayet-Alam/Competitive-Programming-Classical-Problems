...
struct segment_tree{
    ...
    item propagate_op(item par,item child,ll len){
        ...
        ret.lazy=par.lazy;
       if(par.lazy<0){
          ret.value.seg=ret.value.pref=ret.value.suf=0;
          ret.value.sum=len*(par.lazy);
       }
       else{
        ret.value.seg=ret.value.pref=ret.value.suf=ret.value.sum=len*(par.lazy);
       }
       return ret;
    }
    ...
    info get(ll i,ll x,ll lx,ll rx){
        propagate(x,lx,rx);
