...
struct segment_tree{
    ...
    ///RANGE ASSIGN
     void RangeInverse(ll l,ll r,ll x,ll lx,ll rx){
        propagate(x,lx,rx);
        if(lx>=r || l>=rx){
            return;
        }
        if(lx>=l && rx<=r){
            tree[x].lazy=true;
            tree[x].value=(rx-lx)-tree[x].value;
            return;
        }
        ll m=(lx+rx)/2;
        RangeInverse(l,r,2*x+1,lx,m);
        RangeInverse(l,r,2*x+2,m,rx);  
        tree[x].value=merge(tree[2*x+1].value,tree[2*x+2].value);
    }
    void RangeInverse(ll l,ll r){
        //multipliess v from l to r
      RangeInverse(l,r,0,0,size);
    }   
  
    ll find(ll val,ll x,ll lx,ll rx){
        propagate(x,lx,rx);
        if(tree[x].value<=val) return -1; 
        if(rx-lx==1) return lx;
        ll m=(lx+rx)/2;
        ll ret=find(val,2*x+1,lx,m);
        if(ret==-1){
            ret=find(val-tree[2*x+1].value,2*x+2,m,rx);
        }
