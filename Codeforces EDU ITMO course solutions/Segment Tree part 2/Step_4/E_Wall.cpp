...
/*
   hints:
    opertions[nd]  = L..R such that all values shoud be in  this range
*/
struct item  {
    int L  = 0  ,  R =  1e15 ;
...
struct segtree{
    ...
    item operation (item a ,  item b ,  int x){
        ...
        if (r1< l2)           // l1 r1 l2 r2
            return  {l2,l2};
        if (r2<l1)             //l2 r2 l1 r1
           return   {r2,r2};
    ...
    int calc(int  i, int x , int lx , int rx ){
          ...
          if (rx-lx==1){
              ...
              if  (0LL  <= operations[x].L){
                    return  operations[x].L;
              }
              if (0LL>=operations[x] .R){
                    return operations[x].R;
              }
              return 0 ;
          }
          ...
          int val = 0 ;
          if (i<md)
            val = calc(i,  2*x+1 , lx,md);
          else
            val = calc(i, 2*x+2 , md, rx) ;
 
          if (!op[x])
            return val;
          if  (val <= operations[x].L){
                    return  operations[x].L;
          }
          if (val>=operations[x] .R){
                    return operations[x].R;
           }
           return val ;
    }
