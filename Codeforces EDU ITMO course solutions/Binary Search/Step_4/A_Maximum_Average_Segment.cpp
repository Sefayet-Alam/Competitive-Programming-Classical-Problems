...
pair<ll,ll> func(double x){
    vector<double>newvec(n+1,0),pref(n+1,0),m(n+1,0),midx(n+1,0);
...
void bs(double l,double r){
    double eps=1e-9;    
    ll cnt=100;      //set the error limit here
    pair<ll,ll>ans;
    while(cnt--) {
        double mid=(r+l)/2.00;
        if (func(mid).first!=-1){
            ans.first=func(mid).first;
            ans.second=func(mid).second;
            l=mid;
            }
        else r=mid;
    }
    cout<<func(l).first<<" "<<func(l).second<<endl;
}
int main()
{
    fast;
