...
pair<ll, string> findRefrain(const string &s, const vector<ll> &lcp, const vector<ll> &suffix) {
    stack<ll> monoStack;
    const ll n = lcp.size();
    ll maxArea = s.length();
    string res = s;
...
int main()
{
    ...
    auto a=SuffixArray_LCP(s);
    vl suff=a.first;
    vl lcp=a.second;
    for(ll i=0;i<n;i++) lcp[i]=lcp[i+1];
    lcp.pop_back();
    s.pop_back();
    auto ans=findRefrain(s,lcp,suff);
    // deb(lcp);
    cout << ans.first<<nn;
    // deb(ans.second);
    cout<<ans.second.size()<<nn;
