...
vector<ll>SuffixArray_LCP(string &s)
{
    s+='$';
    ll n=s.size();
...
long long int countBorders(const vector<ll> &lcp) {
    long long  n = lcp.size();
    long long  res = (n + 1) * n / 2;  // empty borders for all substrings
...
int main()
{
    string s;
    cin>>s;
    auto lcp=SuffixArray_LCP(s);
    for(ll i=0;i<lcp.size();i++) lcp[i]=lcp[i+1];
    lcp.pop_back();
    ll ans=countBorders(lcp);
    cout<<ans<<nn;
    return 0;
}
