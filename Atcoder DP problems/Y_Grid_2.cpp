...
ll countpaths(ll x1,ll y1,ll x2,ll y2){
    ll ret=nCrMod(x2-x1+y2-y1,x2-x1,M);
    return ret;
}
ll dp[N];
int main()
{
    ...
    sort(all(vec));
    for(ll i=0;i<=n;i++){
        ll curr=countpaths(1,1,vec[i].first,vec[i].second);
        for(ll j=0;j<i;j++){         
            // if(vec[i].second>=vec[j].second){
                curr=(curr-(dp[j]*countpaths(vec[j].first,vec[j].second,vec[i].first,vec[i].second))%M+M)%M;
            // }
        }
        dp[i]=curr;
    }
